...
public class ServerUnitTesting {
    ...
    /**
     * test - runs all the other functions in the ServerUnitTesting class, so
     * the user only have to run this function to do the unit testing.
     *
    ...
    /**
     * setUsername test's the setUsername function of ClientConnection
     *
    ...
    public static boolean addKill() {
        try {
            return ClientConnection.AddOneKill();
        } catch (Exception exc) {
            System.out.println("ServerUnitTest add a kill error: " + exc.toString());
            return false;
        }
