...
\vspace{1cm}
\textbf{Architecture Requirements Specification}
\end{flushleft}
\end{minipage}
~
\begin{minipage}{0.4\textwidth}
...
% % % % % % % % % % % % % % % 
	...
	\newpage
		...
		\begin{tabbing}
			...
			15/10/2014	\> Version 2.1	\> Final read through.							\> Nico Taljaard \\
		\end{tabbing}
	...
	\newpage
		...
		The system will be accessible by users through the following channels:
		\begin{itemize}
			\item From a desktop application running on Windows/Linux operating system.
		...
		\vspace{0.1in}
			...
			\begin{enumerate}
				...
				\item Backend server:
					\begin{itemize}
						...
						\item Database requests should be complete within a 5 second time frame.
					\end{itemize}
			\end{enumerate}
			...
			\begin{enumerate}
				...
				\item Backend Server:
					\begin{itemize}
						...
						\item Should not crash due to over whelming client connections.
			...
			\begin{enumerate}
				\item Multiple maps can be added by adding a scene .j3o file that contains the scene, and a number assignment with the file path.
				\item Support can be added to run multiple servers to distribute client connections.
				\item The server should scale to any amount of client connections.
				\item Multi player functionality can be added using jSpiderMonkey library to have multiple players on the same map.
			\end{enumerate}
			...
			\begin{enumerate}
				\item On login and account password encryption is added by retrieving the users network connection ip, adding it to the password and encrypt with AES.
